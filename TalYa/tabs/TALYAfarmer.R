...
talya_harvest_weekly <- 
  talya %>% group_by(harvest_week) %>%
  summarise(Control=mean(harvest_KG_CONTROL),`Tal-Ya`=mean(harvest_KG_talya100)) %>% 
  mutate(across(is.numeric, round)) 

kable(talya_harvest_weekly)
dt %>%
  kable(talya_harvest_weekly) %>%
  kable_styling()

talya_harvest_weekly <- gather(talya_harvest_weekly, "Group", "value", 2:3)

g_talya_harvest_weekly <- ggplot(data=talya_harvest_weekly, aes(x=harvest_week, y=value, fill=Group)) +
  geom_bar(stat="identity" ,width=0.8, position=position_dodge())+
  theme_minimal()+
  geom_text(
    aes(x = harvest_week, y = value, label = value, group = Group),
...
ggplot(g_revenue,aes(x=plot, y=Revenue, fill=plot)) + 
  ...
  xlab(" ") +
  ylab(" ")+
  geom_text(aes(label=Revenue), vjust=-0.5, colour="black", size=3)+ 
  scale_fill_manual(name="Plot", values=c("#3399FF","#00cc33"))+
...
farmers_by_week <- 
  TALYAfarmer%>%filter(week.%in% c(10,11,12)) %>% 
  filter(!farmer_name %in% c("Vijaya narasimha","A. Muniraghavulu","M. Janardhan naidu","M. Narasimhulu")) %>%
  select(week.,weeknum.year,farmer_name) %>% 
  group_by(week.) %>% count()

ggplot(farmers_by_week,aes(x=week., y=n)) + 
  geom_bar(stat="identity",width=0.4)+
  theme_classic()+
  ggtitle(" ") +
  xlab(" ") +
  ...
  scale_fill_manual(name="Plot", values=c("#00cc33"))+
  theme(legend.position = "none",
        plot.title = element_text(size = rel(1), hjust = 0.5))
...
cost_benefit <- 
  TALYAfarmer%>% inner_join(area_talya_farmers, by = "farmer_name") %>% 
  select(starttime, farmer_name,weeknum.year, 
         transportation_cost_CONTROL,transportation_cost_TALYA100,
         labor_work_days,paid_labor_cost,expense_cost,
           expenses_names, expenses_names2, expenses_names3,revenue_CONTROL,revenue_TALYA100) %>% 
  filter(paid_labor_cost>0 | expense_cost>0 | 
           transportation_cost_CONTROL>0 | transportation_cost_TALYA100>0)

write.csv(cost_benefit,"C:/Users/Dan/Documents/R/TalYa/data/cost_benefit.csv")
cost_benefit <- read.csv("~/R/TalYa/data/cost_benefit.csv")
